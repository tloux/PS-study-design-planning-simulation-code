...
calculate_statistics = function(dataset) { 
  ...
  m_sum_wts0 = mean(dataset$sum_wts0)
  
  m_wts_above_20 = mean(dataset$wts_above_20)
...
weighting_results0 = lapply(weight_sim_list, function(x) calculate_statistics(x))
weighting_results1 = do.call(rbind, weighting_results0)


weighting_results = cbind(parameter_combos, weighting_results1)
...
save(file='weighting-simulations-summary.RData', weighting_results)
